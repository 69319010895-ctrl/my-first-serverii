...
const server = http.createServer(async (req, res) => {  
 ...
 try {  
 // 3. ขอเชื่อมตอและสงคำสั่ง SQL ไปดึงขอมูลจากตาราง students  
 const client = await pool.connect();  
 const result = await client.query('SELECT * FROM students');  
client.release(); // คนืการเชื่อมตอเมื่อใชงานเสร็จ 
// 4. นำขอมูลที่ได(result.rows) มาประกอบเปนตาราง HTML 
 let html = `<h1>ฐานขอมูลนักศึกษา (ทดสอบการเชื่อมตอ)</h1>`;  
 html += `<table border="1" cellpadding="10">`;  
 html += `<tr><th>69319010895</th><th>ปิ่นนภา ลีบ้านลาด</th></tr>`;  
 // วนลูปนำขอมูลแตละแถวมาแสดง  
